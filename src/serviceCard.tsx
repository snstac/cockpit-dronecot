...
export function ServiceManagementCard({ serviceName, onToast }: ServiceManagementCardProps) {
    ...
    async function runCtl(action: string, label: string) {
        try {
            ...
            if (/(\bcode \b4\b|exit status 4|exited with code 4)/i.test(detail)) {
                detail = _(
                    'Unit {0}.service was not found. Install the dronecot package (or create the unit), then refresh this page.'
    ...
    return (
        <Card data-testid="dronecot-service-card">
            <CardTitle>
            ...
            <CardBody>
                ...
                {unitNotFound && !error && (
                    <Alert
                        variant="warning"
                        title={_('DRONECOT service unit is not installed')}
                    >
                        {_(
                            'systemd has no {0}.service. Install the dronecot package on this machine (see documentation), run daemon-reload if you added a unit file, then refresh this page.'
                        ).replace('{0}', serviceName)}
                    </Alert>
                )}
