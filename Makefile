# Cockpit DRONECOT — build and install.
#
# Quick start:
#   make                  # build the plugin into dist/ (fetches pkg/lib, npm ci)
#   sudo make install     # install plugin system-wide (/usr/share/cockpit/dronecot)
...
install: $(DIST_TEST) po/LINGUAS
	...
	install -m 0644 packaging/49-cockpit-dronecot.rules \
		$(DESTDIR)$(PREFIX)/share/polkit-1/rules.d/49-cockpit-dronecot.rules
	@echo "Installed plugin -> $(COCKPITDIR)"

uninstall:
	rm -rf $(COCKPITDIR)
	rm -f $(DESTDIR)$(PREFIX)/share/metainfo/$(APPSTREAMFILE)
	rm -f $(DESTDIR)$(PREFIX)/share/polkit-1/rules.d/49-cockpit-dronecot.rules
